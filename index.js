...
// Initialize Split.js for resizable split views
Split(['.col-content:first-child', '.col-content:last-child'], {
    sizes: [50, 50], 
    minSize: [100, 100], 
    gutterSize: 10, 
    cursor: 'col-resize', 
...
// Add event listeners to the textarea for focus and blur
...
//     convertButton.innerText = 'Convert to code';
// });
...
codeTextarea.addEventListener('focus', function () {
	...
	convertToTextButton.style.display = 'block'
});
...
ClassicEditor.create(document.querySelector('#cke5-feature-rich-demo'), {
	...
	.then((editor) => {
		editor.editing.view.document.on('focus', () => {
            ...
            convertButton.style.display = 'block';
            convertToTextButton.style.display = 'none';
        });
		document.getElementById('convertButton').addEventListener('click', function () {
            const featureRichDemoContent = editor.getData();
            document.getElementById('codeTextarea').value = `<!DOCTYPE html>
                <html>
                    <head>
                    ...
                    <body>
                        ${featureRichDemoContent}
                    </body>
                </html>`;
        });

		document.getElementById('convertToTextButton').addEventListener('click', function () {
            const htmlCode = document.getElementById('codeTextarea').value;
			console.log(htmlCode);
            editor.setData(htmlCode);
        });
    })
	.catch((error) => {
